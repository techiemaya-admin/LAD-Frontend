...
export function useCampaignOrchestrator(
  ...
  const startFlow = useCallback(async () => {
    ...
    try {
      ...
      console.error('[CampaignOrchestrator] Error starting flow:', error);
      addAIMessage({
        role: 'ai',
  ...
  const handleAnswer = useCallback(async (userInput: string) => {
    ...
    try {
      ...
      console.error('[CampaignOrchestrator] Error processing answer:', error);
      addAIMessage({
        role: 'ai',
  ...
  const moveToStep = useCallback(async (step: number, currentState: CampaignState) => {
    try {
      ...
      console.error('[CampaignOrchestrator] Error moving to step:', error);
    } finally {
      setIsLoading(false);
      setIsProcessingAI(false);
    }
