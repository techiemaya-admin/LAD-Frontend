...
import { getDecisionMakerPhone } from '../services/apolloPhoneService';
...
if (!id) return null;
  ...
  const handleSelectAll = () => {
    if (selectAll) {
      ...
      const allIds = new Set(data.map((company, index) => company.id || index));
  ...
  const handleStartIntelligentCallingEmployees = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/voiceagent/resolve-phones`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ids: employeeIds, type: 'employee' }),
      });
      ...
      // Prefer IDs returned by API, but fall back to originally selected IDs if missing
      const apiIds = Array.isArray(json.data)
        ? json.data.map(row => row.employee_id || row.id).filter(Boolean)
        : [];
      const idsToForward = apiIds.length > 0 ? apiIds : employeeIds;
      const encodedIds = btoa(JSON.stringify(idsToForward));

      push({ title: 'Success', description: `${json.data.length} target(s) resolved. Redirecting to call...` });
      router.push(`/make-call?ids=${encodedIds}&bulk=1&prefilled=1&type=employee`);
    } catch (error) {
      console.error('Intelligent calling failed for employees:', error);
      push({ variant: 'error', title: 'Error', description: error.message || 'Failed to start calling.' });
    } finally {
      setIntelligentCallingLoading(false);
    }
...
// };
      ...
      if (selectedAvailabilityFilters.length > 0) {
        ...
        for (const filterType of selectedAvailabilityFilters) {
          ...
          switch (filterType) {
            ...
            case 'with-summary':
              // Check if company has a sales summary (not null, not empty, not "not related")
              ...
              matchesThisAvailability = Boolean(
                summary && 
                summary !== "not related" && 
  ...
  const handleGetContact = async (company) => {
    ...
    try {
      ...
      if (phoneResult.data && (phoneResult.data.phoneNumber || phoneResult.data.phone)) {
        phoneInfo = {
          phone: phoneResult.data.phoneNumber || phoneResult.data.phone,
      ...
      // Format 2: Direct fields at top level
        ...
        phoneInfo = {
          phone: phoneResult.phoneNumber || phoneResult.phone,
      ...
      // Format 3: Old format (nested people array)
        ...
        if (firstPhone) {
          phoneInfo = {
            phone: firstPhone.sanitized_number || firstPhone.raw_number,
  ...
  const handleGetEmployees = async (company) => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/apollo-leads/get-employees`, {
        method: 'POST',
        headers: {
      ...
      if (data.success && data.employees && data.employees.length > 0) {
        ...
        setEmployeeError(prev => ({ ...prev, [companyId]: 'No employees found' }));
        
        // Open dialog even when no employees found to show the message
        setSelectedEmployeeCompany(company);
        setEmployeeDialogOpen(true);
      }
    } catch (error) {
      ...
      setEmployeeError(prev => ({ 
        ...prev, 
        [companyId]: error.message || 'Failed to get employees' 
      }));
    } finally {
  ...
  const handleRevealPhone = async (employee) => {
    ...
    if (revealedContacts[employeeId]?.phone) {
  ...
  const processPhoneReveal = async (employee) => {
    ...
    try {
      ...
      if (employee.phone && employee.phone.trim() !== '') {
        console.log('✅ Phone already available in employee data:', employee.phone);
        setRevealedContacts(prev => ({
          ...prev,
          [employeeId]: { ...prev[employeeId], phone: employee.phone }
        }));
        ...
        const pollInterval = setInterval(async () => {
          ...
          try {
            ...
            if (statusData.success && statusData.phone) {
              ...
              setRevealedContacts(prev => ({
                ...prev,
                [employeeId]: { ...prev[employeeId], phone: statusData.phone }
              }));
              ...
              // Phone number not available - no alert needed (UI shows loading stopped)
            ...
            if (pollAttempts >= maxPollAttempts) {
              ...
              // Timeout - no alert needed (UI shows loading stopped)
      ...
      if (error.message.includes('404') || error.message.includes('not found') || error.message.includes('not available')) {
  ...
  const handleRevealEmail = async (employee) => {
    ...
    // Check if already revealed
    if (revealedContacts[employeeId]?.email) {
  ...
  return (
    <Box sx={{ width: '100%', p: 2 }}>
      ...
      {!searchQuery && (
        <Box sx={{ mb: 3, display: 'flex', alignItems: 'center', gap: 2 }}>
          ...
          <Typography variant="h4" fontWeight="bold">
            Company Data Results ({data.length} companies found)
          </Typography>
        </Box>
      )}

      {/* Tabs for Companies and Employees - Always show if either has data */}
      {(data.length > 0 || employeeData.length > 0) && (
        <Box sx={{ borderBottom: 1, borderColor: 'divider', mb: 3 }}>
          ...
          >
            <Tab 
              label={`Companies (${data.length})`} 
              icon={<Business />}
              iconPosition="start"
            />
            <Tab 
              label={`Employees (${employeeData.length})`} 
              icon={<Person />}
              iconPosition="start"
            />
          </Tabs>
      ...
      <Box sx={{ p: 2, mb: 3, position: 'relative' }}>
          ...
          {/* AI Trigger Button - Discount dropdown removed */}
          <Fade in={showAITrigger}>
            <Box sx={{ display: 'flex', justifyContent: 'flex-end' }}>
        <Button
          variant="contained"
          startIcon={<PhoneIcon />}
          onClick={activeTab === 0 ? handleStartIntelligentCallingCompanies : handleStartIntelligentCallingEmployees}
          disabled={intelligentCallingLoading || (activeTab === 0 ? selectedCompanies.size === 0 : selectedEmployees.size === 0)}
          sx={{ background: '#0b1957', borderRadius: '20px', '&:hover': { background: '#0d1f6f' } }}
        >
      ...
      >
        <MenuItem disabled sx={{ fontWeight: 700, color: '#0b1957', fontSize: '0.875rem', opacity: 1, bgcolor: 'oklch(0.97 0 0)' }}>
        ...
        <MenuItem disabled sx={{ fontWeight: 700, color: '#0b1957', fontSize: '0.875rem', opacity: 1, bgcolor: 'oklch(0.97 0 0)', mt: 1 }}>
      ...
      {data.length > 0 && activeTab === 0 ? (
      <Box sx={{ display: 'grid', gridTemplateColumns: { xs: 'repeat(1, 1fr)', sm: 'repeat(2, 1fr)', md: 'repeat(4, 1fr)' }, gap: 2, position: 'relative', zIndex: 2 }}>
        {[...getFilteredData()]
          ...
          .map((company, index) => (
          <Box key={company.id || index} sx={{ display: 'flex', flexDirection: 'column', alignItems: 'stretch', height: '100%' }}>
            <Card 
                ...
                {company.employeeCount !== undefined && company.employeeCount !== null ? (
                  <Box sx={{ mt: 1.5, display: 'flex', flexWrap: 'wrap', gap: 0.5 }}>
                    {parseInt(company.employeeCount) >= 200 && (
                      <Chip
                        icon={<Tag sx={{ fontSize: 14, color: '#6c757d' }} />}
                        label="#enterprise"
                        size="small" 
                        variant="outlined"
                        sx={{ 
      ...
      {activeTab === 1 && (
        <Box sx={{ p: 2 }}>
          ...
          {/* {console.log('👥 Employees Tab Content - activeTab:', activeTab, 'employeeData.length:', employeeData.length)} */}
          
          {/* Selection Controls - Same as Company Page */}
          <Box sx={{ p: 2, mb: 3 }}>
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, flexWrap: 'wrap' }}>
                  ...
                  <Checkbox
                    ...
                    onChange={() => {
                      if (selectAllEmployees) {
                        ...
                        const allIndices = new Set(employeeData.map((_, index) => index));
                        if (onEmployeeSelectionChange) {
                          onEmployeeSelectionChange(allIndices);
                        }
              ...
              <Typography variant="body2" sx={{ color: 'oklch(0.145 0 0)' }}>
                {selectedEmployees.size} of {employeeData.length} employees selected
            </Typography>
            
              {/* Unlock Buttons for Selected Employees */}
            {selectedEmployees.size > 0 && (
                <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap', ml: 'auto' }}>
                  {onUnlockEmployeeEmails && (
                <Button
                  variant="contained"
                  size="small"
                  startIcon={<Email />}
                  onClick={onUnlockEmployeeEmails}
                  ...
                  {onUnlockEmployeePhones && (
                <Button
                  variant="contained"
                  color="secondary"
                  size="small"
                  startIcon={<Phone />}
                  onClick={onUnlockEmployeePhones}
                  ...
                  {/* AI Trigger Button for Employees - Moved to last */}
                  <Fade in={selectedEmployees.size > 0}>
                    <Button
                      variant="contained"
                      color="secondary"
                      startIcon={<SmartToy />}
                      onClick={async () => {
                        const selectedEmployeeData = employeeData.filter((employee, index) =>
                          selectedEmployees.has(index)
                        );
                        
                        try {
                          // Build phone data: prioritize employee phone, fallback to company phone
                          const phoneDataList = selectedEmployeeData.map(employee => {
                            // Try to parse employee_data JSON if available
                            ...
                            // 1. First check employee phone (from revealed contacts or employee data)
                            const employeeId = employee.id || `${employee.first_name}_${employee.last_name}`;
                            const revealed = revealedEmployeeContacts[employeeId];
                            let phone = revealed?.phone || employee.phone || '';
                            
                            // Check if phone is locked or empty
                            const hasEmployeePhone = phone && 
                              phone !== '🔒 Locked' &&
                              phone.trim().length > 0 &&
                              phone !== 'null' &&
                              phone !== 'undefined';
                            
                            // 2. If no employee phone, check company phone
                            if (!hasEmployeePhone) {
                            ...
                            return {
                              phone: phone && phone !== '🔒 Locked' && phone.trim() ? phone.trim() : null,
                              name: name,
                              employeeId: employee.apollo_person_id || employee.id,
                              hasPhone: !!(phone && phone !== '🔒 Locked' && phone.trim() && phone !== 'null' && phone !== 'undefined')
                            };
                          }).filter(item => item.hasPhone);

                          console.log("📞 Phone data extracted:", phoneDataList);

                          if (phoneDataList.length === 0) {
                            alert("No phone numbers found for the selected employees. Please unlock phone numbers first or ensure companies have phone numbers.");
                            return;
                          }

                          const ERP_URL = process.env.NEXT_PUBLIC_ERP_URL || "https://erp.techiemaya.com";

                          if (phoneDataList.length === 1) {
                            // ✅ single call via query params
                            const first = phoneDataList[0];
                            const href = new URL(`${ERP_URL}/make-call`);
                            href.searchParams.set("dial", String(first.phone || "").trim());
                            href.searchParams.set("clientName", String(first.name || "").trim());
                            href.searchParams.set("prefilled", "1");
                          ...
                          } else if (phoneDataList.length > 1) {
                            // ✅ bulk flow — pass phone numbers directly
                            const phoneNumbers = phoneDataList.map(p => p.phone).filter(Boolean);
                            const encodedPhones = encodeURIComponent(btoa(JSON.stringify(phoneNumbers)));
                            const href = `${ERP_URL}/make-call?bulk=1&phones=${encodedPhones}`;
                            console.log("📞 Opening bulk call URL:", href);
                            const newWindow = window.open(href, "_blank");
                            if (!newWindow) {
                              alert("Popup blocked! Please allow popups for this site to open the calling interface.");
                          ...
                          console.error("❌ Error triggering AI calls for employees:", error);
                          alert(`Error: ${error.message || "Failed to initiate calling. Please try again."}`);
                        }
                      }}
                      sx={{
                        background: '#0b1957',
                    ...
                    >
                      Start Intelligent Calling ({selectedEmployees.size})
                    </Button>
                  </Fade>
              </Box>
            )}
          ...
          {employeeData.length > 0 && activeTab === 1 ? (
          <Box sx={{ display: 'grid', gridTemplateColumns: { xs: 'repeat(1, 1fr)', sm: 'repeat(2, 1fr)', md: 'repeat(4, 1fr)' }, gap: 2, position: 'relative', zIndex: 2 }}>
                    ...
                    <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', minWidth: 120 }}>
                      <Box sx={{ position: 'relative', display: 'inline-flex', mb: 1 }}>
                        ...
                        >
                          ...
                          >
                            <People sx={{ color: '#3a7bd5', fontSize: 32, mb: 0.5 }} />
                            <Typography variant="caption" fontWeight="bold" sx={{ 
                              color: '#3a7bd5',
                              fontSize: '0.7rem',
                              textAlign: 'center',
        ...
        <DialogContent dividers>
          <Grid container spacing={2}>
            {selectedEmployeeCompany && filterEmployeesByRole(fetchedEmployeeData[selectedEmployeeCompany?.id || selectedEmployeeCompany.domain], employeeRoleFilter)?.map((employee, index) => (
              <Grid item xs={12} sm={employeeViewMode === 'grid' ? 6 : 12} md={employeeViewMode === 'grid' ? 4 : 12} key={index}>
                <Card sx={{ 
                  height: '100%',
                  background: '#ffffff',
                  borderRadius: '12px',
                  ...
                  <CardContent sx={{ p: employeeViewMode === 'grid' ? 3 : 2.5 }}>
                    <Box sx={{ 
                      display: 'flex', 
                      flexDirection: employeeViewMode === 'grid' ? 'column' : 'row',
                      alignItems: employeeViewMode === 'grid' ? 'center' : 'center',
                      gap: employeeViewMode === 'grid' ? 1.5 : 4,
                      justifyContent: employeeViewMode === 'list' ? 'space-between' : 'flex-start'
                    }}>
                      ...
                      <Box sx={{ 
                        ...
                        <Avatar 
                          ...
                          sx={{ 
                            ...
                            border: employeeViewMode === 'grid' ? '4px solid' : '3px solid',
                            borderColor: '#0b1957',
                            boxShadow: employeeViewMode === 'grid' ? 2 : 3,
                            flexShrink: 0
                          }}
                        >
                          <Person sx={{ fontSize: employeeViewMode === 'grid' ? 50 : 40 }} />
                        </Avatar>
                      ...
                      {/* Employee Details Wrapper (Grid View) */}
                      <Box sx={{ 
                        display: employeeViewMode === 'grid' ? 'flex' : 'none',
                        flexDirection: 'column', 
                        alignItems: 'center',
                        gap: 1.5,
                        width: '100%'
                      ...
                      {/* Employee Name */}
                      <Typography variant="h6" align={employeeViewMode === 'grid' ? 'center' : 'left'} sx={{ 
                        fontWeight: 'bold', 
                        fontSize: '1.1rem',
                        color: '#0b1957',
                      ...
                      {employee.company_name && (
                        <Box sx={{ 
                          ...
                          <Typography variant="caption" sx={{ fontWeight: 600, color: '#0b1957' }}>
                            @ {employee.company_name}
                          </Typography>
                        </Box>
                      )}
                      ...
                      <Box sx={{ width: '100%', display: 'flex', flexDirection: 'column', gap: 1.5 }}>
                        ...
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                          ...
                          <Typography variant="caption" sx={{ 
                            color: revealedContacts[employee.id || employee.name]?.phone ? '#0b1957' : 'oklch(0.556 0 0)',
                            fontSize: '0.8rem',
                            letterSpacing: revealedContacts[employee.id || employee.name]?.phone ? 'normal' : '1px',
                            filter: revealedContacts[employee.id || employee.name]?.phone ? 'none' : 'blur(4px)',
                            userSelect: revealedContacts[employee.id || employee.name]?.phone ? 'text' : 'none',
                            flex: 1,
                            fontWeight: revealedContacts[employee.id || employee.name]?.phone ? 600 : 400
                          }}>
                            {revealedContacts[employee.id || employee.name]?.phone || '+971 50 123 4567'}
                          </Typography>
                          <Tooltip title={revealedContacts[employee.id || employee.name]?.phone ? "Phone number revealed" : "Click to reveal phone number"} arrow>
                            <IconButton
                              size="small"
                              onClick={(e) => {
                                e.stopPropagation();
                              ...
                              disabled={revealingContacts[employee.id || employee.name]?.phone || revealedContacts[employee.id || employee.name]?.phone}
                              sx={{ 
                                bgcolor: revealedContacts[employee.id || employee.name]?.phone ? 'oklch(0.97 0 0)' : 'oklch(0.97 0 0)',
                                border: '1px solid oklch(0.922 0 0)',
                                '&:hover': { bgcolor: revealedContacts[employee.id || employee.name]?.phone ? 'oklch(0.97 0 0)' : 'oklch(0.97 0 0)', borderColor: '#0b1957' },
                                p: 0.5
                              }}
                            >
                              {revealingContacts[employee.id || employee.name]?.phone ? (
                                  <CircularProgress size={20} sx={{ color: '#0b1957' }} />
                              ) : revealedContacts[employee.id || employee.name]?.phone ? (
                        ...
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                          ...
                          <Typography variant="caption" sx={{ 
                            color: revealedContacts[employee.id || employee.name]?.email ? '#0b1957' : 'oklch(0.556 0 0)',
                            fontSize: '0.8rem',
                            letterSpacing: revealedContacts[employee.id || employee.name]?.email ? 'normal' : '1px',
                            filter: revealedContacts[employee.id || employee.name]?.email ? 'none' : 'blur(4px)',
                            userSelect: revealedContacts[employee.id || employee.name]?.email ? 'text' : 'none',
                            overflow: 'hidden',
                            textOverflow: 'ellipsis',
                            whiteSpace: 'nowrap',
                            flex: 1,
                            fontWeight: revealedContacts[employee.id || employee.name]?.email ? 600 : 400
                          }}>
                            {revealedContacts[employee.id || employee.name]?.email || 'name@company.com'}
                          </Typography>
                          <Tooltip title={revealedContacts[employee.id || employee.name]?.email ? "Email address revealed" : "Click to reveal email address"} arrow>
                            <IconButton
                              size="small"
                              onClick={(e) => {
                                e.stopPropagation();
                              ...
                              disabled={revealingContacts[employee.id || employee.name]?.email || revealedContacts[employee.id || employee.name]?.email}
                              sx={{ 
                                bgcolor: revealedContacts[employee.id || employee.name]?.email ? 'oklch(0.97 0 0)' : 'oklch(0.97 0 0)',
                                border: '1px solid oklch(0.922 0 0)',
                                '&:hover': { bgcolor: revealedContacts[employee.id || employee.name]?.email ? 'oklch(0.97 0 0)' : 'oklch(0.97 0 0)', borderColor: '#0b1957' },
                                p: 0.5
                              }}
                            >
                              {revealingContacts[employee.id || employee.name]?.email ? (
                                  <CircularProgress size={20} sx={{ color: '#0b1957' }} />
                              ) : revealedContacts[employee.id || employee.name]?.email ? (
                                  <CheckCircle sx={{ fontSize: 20, color: '#0b1957' }} />
                              ) : (
                                  <Lock sx={{ fontSize: 20, color: '#0b1957' }} />
                              )}
                      ...
                      </Box> {/* End Employee Details Wrapper (Grid View) */}
                          ...
                          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.2 }}>
                            ...
                            <Tooltip title={revealedContacts[employee.id || employee.name]?.phone ? "Phone number revealed" : "Click to reveal phone number"} arrow>
                              <IconButton 
                                ...
                                sx={{ 
                                  p: 0.5,
                                  bgcolor: revealedContacts[employee.id || employee.name]?.phone ? '#c8e6c9' : '#e3f2fd',
                                  '&:hover': { bgcolor: revealedContacts[employee.id || employee.name]?.phone ? '#c8e6c9' : '#bbdefb' }
                                }}
                              >
                                {revealingContacts[employee.id || employee.name]?.phone ? (
                          ...
                          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.2 }}>
                            ...
                            <Tooltip title={revealedContacts[employee.id || employee.name]?.email ? "Email address revealed" : "Click to reveal email address"} arrow>
                              <IconButton 
                                ...
                                sx={{ 
                                  p: 0.5,
                                  bgcolor: revealedContacts[employee.id || employee.name]?.email ? '#c8e6c9' : '#e8f5e9',
                                  '&:hover': { bgcolor: revealedContacts[employee.id || employee.name]?.email ? '#c8e6c9' : '#c8e6c9' }
                                }}
                              >
                                {revealingContacts[employee.id || employee.name]?.email ? (
                                  <CircularProgress size={16} sx={{ color: '#0b1957' }} />
          ...
          {selectedEmployeeCompany && 
           ...
           (!fetchedEmployeeData[selectedEmployeeCompany.id || selectedEmployeeCompany.domain] || 
            ...
            <Box sx={{ textAlign: 'center', py: 6 }}>
              ...
              <Typography variant="body2" color="text.secondary" sx={{ mt: 1 }}>
                This company doesn't have any employees in Apollo's database.
              </Typography>
            </Box>
          )}
                ...
                <Box sx={{ 
                  ...
                  <Box sx={{ width: '100%', display: 'flex', flexDirection: 'column', gap: 1.5 }}>
                    ...
                    {(() => {
                      ...
                      const empRevealed = revealedEmployeeContacts[empId] || {};
                      const empRevealing = unlockingEmployeeContacts[empId] || {};
                      ...
                      return (
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                          ...
                          <Typography variant="caption" sx={{ 
                            color: empRevealed.phone ? '#0b1957' : 'oklch(0.556 0 0)',
                            fontSize: '0.8rem',
                            letterSpacing: empRevealed.phone ? 'normal' : '1px',
                            filter: empRevealed.phone ? 'none' : 'blur(4px)',
                            userSelect: empRevealed.phone ? 'text' : 'none',
                            flex: 1,
                            fontWeight: empRevealed.phone ? 600 : 400
                          }}>
                            {empRevealed.phone || selectedEmployee.phone_number || '+971 50 123 4567'}
                          </Typography>
                          <Tooltip title={empRevealed.phone ? "Phone number revealed" : "Click to reveal phone number"} arrow>
                            <IconButton
                              size="small"
                              onClick={(e) => {
                                e.stopPropagation();
                              ...
                              disabled={empRevealing?.phone || empRevealed.phone}
                              sx={{ 
                                bgcolor: 'oklch(0.97 0 0)',
                                border: '1px solid oklch(0.922 0 0)',
                    ...
                    {(() => {
                      ...
                      return (
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                          ...
                          <Typography variant="caption" sx={{ 
                            color: empRevealed.email ? '#0b1957' : 'oklch(0.556 0 0)',
                            fontSize: '0.8rem',
                            letterSpacing: empRevealed.email ? 'normal' : '1px',
                            filter: empRevealed.email ? 'none' : 'blur(4px)',
                            userSelect: empRevealed.email ? 'text' : 'none',
                            overflow: 'hidden',
                            textOverflow: 'ellipsis',
                            whiteSpace: 'nowrap',
                            flex: 1,
                            fontWeight: empRevealed.email ? 600 : 400
                          }}>
                            {empRevealed.email || selectedEmployee.email || 'name@company.com'}
                          </Typography>
                          <Tooltip title={empRevealed.email ? "Email address revealed" : "Click to reveal email address"} arrow>
                            <IconButton
                              size="small"
                              onClick={(e) => {
                                e.stopPropagation();
                              ...
                              disabled={empRevealing?.email || empRevealed.email}
                              sx={{ 
                                bgcolor: 'oklch(0.97 0 0)',
                                border: '1px solid oklch(0.922 0 0)',
                                '&:hover': { bgcolor: 'oklch(0.97 0 0)', borderColor: '#0b1957' },
