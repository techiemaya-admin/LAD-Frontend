...
import { CARD, CTA_PRIMARY, INPUT_FOCUS, LINK, PANEL, TINT } from './studio-theme';
...
const LEVEL_TONE: Record<string, string> = { strong: TINT.readyText, moderate: TINT.warnText, weak: TINT.neededText };
    ...
    <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]">
      <section className="space-y-3">
        <div className={`${CARD} p-4`}>
          <h3 className="text-sm font-semibold tracking-tight">Add a sample lead</h3>
          ...
          <div className="mt-2 grid gap-2 sm:grid-cols-2">
            <Input placeholder="Name *" value={lead.name ?? ''} onChange={e => setLead({ ...lead, name: e.target.value })} className={INPUT_FOCUS} />
            <Input placeholder="Title / headline" value={lead.title ?? ''} onChange={e => setLead({ ...lead, title: e.target.value })} className={INPUT_FOCUS} />
            <Input placeholder="Company" value={lead.company ?? ''} onChange={e => setLead({ ...lead, company: e.target.value })} className={INPUT_FOCUS} />
            <Input placeholder="Location" value={lead.location ?? ''} onChange={e => setLead({ ...lead, location: e.target.value })} className={INPUT_FOCUS} />
          </div>
          <Textarea className={`mt-2 ${INPUT_FOCUS}`} rows={2} placeholder="About / summary (optional)" value={lead.summary ?? ''} onChange={e => setLead({ ...lead, summary: e.target.value })} />
          <Button size="sm" className={`mt-2 ${CTA_PRIMARY}`} onClick={addAndScore} disabled={score.isPending || !lead.name?.trim()}>
          ...
          {samples.map(s => (
            <li key={s.id} className={`${CARD} p-3`}>
              <div className="flex items-start gap-2">
                ...
                <button type="button" onClick={() => setSamples(x => x.filter(y => y.id !== s.id))} className="text-muted-foreground transition-colors duration-150 hover:text-foreground" aria-label="Remove"><Trash2 className="h-4 w-4" /></button>
              </div>
        ...
        {result && (
          <>
            ...
            {result.profileSuggestions && result.profileSuggestions.length > 0 && (
              <div className={`${CARD} p-4`}>
                <h4 className="text-sm font-semibold tracking-tight">Suggested profile edits</h4>
                <p className="mt-0.5 text-xs text-muted-foreground">These are answers in your Business Profile, not part of the change above. Edit them on the{' '}
                  <Link href="/settings?tab=businessprofile" className={LINK}>Business Profile</Link> page.</p>
