...
export default function RehearsalRoom({ ready, draft, onDraft }: { ready: boolean; draft?: Overlay; onDraft: (o?: Overlay) => void }) {
  ...
  if (!ready) {
    return <p className={`${PANEL} p-4 text-sm text-muted-foreground`}>The rehearsal room opens once your LinkedIn agent prompt exists. See the status above.</p>;
  }
  ...
  return (
    <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]">
      <section className="space-y-3">
        <div className={`${CARD} p-4`}>
          ...
          <div className="grid gap-2 sm:grid-cols-3">
            ...
            <Input placeholder="Role" value={persona.role ?? ''} onChange={e => setPersona({ ...persona, role: e.target.value })} className={INPUT_FOCUS} />
            <Input placeholder="Company" value={persona.company ?? ''} onChange={e => setPersona({ ...persona, company: e.target.value })} className={INPUT_FOCUS} />
          </div>
          <Textarea className={`mt-2 ${INPUT_FOCUS}`} rows={2} placeholder="Their situation (what the agent could plausibly know)" value={persona.situation ?? ''} onChange={e => setPersona({ ...persona, situation: e.target.value })} />
        </div>
        ...
        <div className={CARD}>
            ...
            {transcript.map((t, i) => (
              <div key={i} className={`flex ${t.role === 'prospect' ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[85%] whitespace-pre-wrap px-3 py-2 text-sm ${t.role === 'prospect' ? `rounded-2xl rounded-br-md ${BUBBLE_ME}` : `rounded-2xl rounded-bl-md ${BUBBLE_AGENT}`}`}>
                  <div className={`mb-0.5 text-[10px] uppercase tracking-wide ${t.role === 'prospect' ? 'opacity-80' : 'text-muted-foreground'}`}>{t.role === 'prospect' ? persona.name : 'Your agent'}</div>
                  {t.content}
                </div>
              </div>
            ))}
          ...
          <div className={`flex gap-2 border-t ${BORDER} p-3`}>
            <Textarea rows={2} value={message} onChange={e => setMessage(e.target.value)}
              ...
              placeholder={`Say something as ${persona.name?.split(' ')[0] || 'the prospect'}…`} disabled={rehearse.isPending} className={INPUT_FOCUS} />
      ...
      <section className="space-y-3">
        <div className={`${CARD} p-4`}>
          <h3 className="text-sm font-semibold tracking-tight">What should the agent have done differently?</h3>
