...
export const BusinessHoursModal: React.FC<BusinessHoursModalProps> = ({ initialSummary, onSave, onClose }) => {
  const [startTime, setStartTime] = useState('09:00');
  const [endTime, setEndTime] = useState('18:00');
  const [timezone, setTimezone] = useState('GST+4');
              ...
              <div key={label}>
                ...
                <div className="relative">
                  <input
                    ...
                    className="w-full bg-gray-50 border border-gray-200 rounded-xl px-3.5 py-2.5 pr-10 text-sm font-medium text-gray-800 outline-none focus:border-[#0B1957] focus:ring-2 focus:ring-[#0B1957]/10 transition-all appearance-none [color-scheme:light]"
                  />
            ...
            {DAYS.map((d, i) => {
              ...
              return (
                <button
                  key={d}
                  onClick={() => toggleDay(i)}
                  className={`aspect-square rounded-xl text-[11px] font-semibold border transition-all flex flex-col items-center justify-center gap-0.5 ${
                    active
