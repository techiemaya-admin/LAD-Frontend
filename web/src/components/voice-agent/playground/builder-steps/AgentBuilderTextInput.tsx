...
export function AgentBuilderTextInput({
  ...
  return (
    <div className="relative flex flex-col items-center w-[448px] max-w-full h-[550px] bg-white dark:bg-[#000724] rounded-3xl border border-slate-200 dark:border-blue-950/40 shadow-xl overflow-hidden animate-in fade-in zoom-in-95 duration-300 outline-none focus:outline-none focus:ring-0">
      ...
      <div className="relative flex-1 min-h-0 w-full flex flex-col pt-6">
         <div className="flex-grow overflow-y-auto scrollbar-none px-6 pb-12">
            <h2 className="text-xl md:text-2xl font-bold text-[#0b1957] dark:text-slate-100 text-center leading-snug">
               {question}
            </h2>
            {description && (
               <div className="mt-4 text-xs md:text-sm text-slate-500 dark:text-slate-400 text-center leading-relaxed font-medium">
                 <ReactMarkdown
                   components={{
                     ...
                     li: ({ node, ...props }) => <li className="text-slate-500 dark:text-slate-400 font-medium" {...props} />,
                   }}
                 >
                   {description}
                 </ReactMarkdown>
      ...
      {enableUpload && (references.length > 0 || isUploading) && (
        <div className="w-full flex flex-col px-8 mb-2 z-10 space-y-1 animate-in fade-in duration-200">
          ...
          {references.length > 0 && (
            <div className="text-[10px] text-slate-500 dark:text-slate-400 font-medium italic mb-1 animate-in fade-in duration-200">
      ...
      <div className="w-full flex flex-col mt-auto pb-6 pt-2 bg-gradient-to-t from-white dark:from-[#000724] via-white dark:via-[#000724] to-transparent relative z-20">
         {showSkip && (
            <div className="w-full flex justify-end px-6 -mb-1 z-10">
              <button
                 type="button"
                 onClick={() => onNext?.("")}
                 className="px-4 py-1.5 rounded-full border border-slate-200 dark:border-blue-950/40 text-[10px] font-semibold text-slate-400 hover:bg-slate-50 dark:hover:bg-blue-950/40 hover:text-[#0b1957] dark:hover:text-white transition-all uppercase tracking-wider cursor-pointer shadow-sm"
              >
                Skip
              </button>
            </div>
