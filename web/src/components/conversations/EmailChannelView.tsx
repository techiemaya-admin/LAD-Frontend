            ...
            <div className="max-h-60 overflow-y-auto custom-scrollbar">
                {loading ? (
                    ...
                    filtered.map(tpl => (
                        ...
                        >
                            <div
                                className="h-8 w-8 rounded-lg bg-blue-50 flex items-center justify-center flex-shrink-0">
                    ...
                    <>
                        <div className="max-h-60 overflow-y-auto p-3 space-y-1">
                            ...
                            {groups.filter(g => g.channel === provider).length === 0 && (
                                <p className="text-xs text-muted-foreground text-center py-4">No broadcast groups
                                    yet</p>
                            )}
                        </div>
...
function ContactDetailsPanel({contact, provider, groups, onClose, onAddToGroup}: ContactDetailsPanelProps) {
    ...
    return (
        <div
            className="w-72 flex-shrink-0 flex flex-col border-l border-border bg-card overflow-y-auto custom-scrollbar">
            {/* Header */}
            <div className="px-4 py-3 border-b border-border flex items-center justify-between">
                <span
                    className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Contact Details</span>
                <button
                    onClick={onClose}
            ...
            <div className="px-4 py-4 border-b border-border">
                <div className="flex items-center justify-between mb-2">
                    <span
                        className="text-[11px] font-semibold text-muted-foreground uppercase tracking-wider">Labels</span>
                    <button
            ...
            <div className="px-4 py-4">
                <span
                    className="text-[11px] font-semibold text-muted-foreground uppercase tracking-wider">Metadata</span>
...
function EmailComposePanel({contact, provider, onShowDetails, showDetails}:
    ...
    const uploadAttachmentToGCS = async (file: File) => {
        ...
        // Get your standard headers
        const headers = authHeaders();

        // CRITICAL: Delete Content-Type if it exists so the browser can
        ...
        const response = await fetch(`${API}/upload`, {
            method: 'POST',
            headers: {
                ...headers, // Now it only contains Authorization, etc.
            },
            body: formData,
        });
            ...
            <div className="h-16 px-4 flex items-center gap-3 bg-card border-b border-border flex-shrink-0">
                ...
                <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground">
                        {PROVIDER_ICON[provider]} {PROVIDER_LABEL[provider]} broadcast
...
export function EmailChannelView({provider, connectedEmail}: EmailChannelViewProps) {
    ...
    useEffect(() => {
        loadContacts();
        loadGroups();
    }, [loadContacts, loadGroups, groupRefreshKey]);
    ...
    return (
        <div className="flex-1 flex overflow-hidden">
              ...
              Contacts ({contacts.length})
                    ...
                    <div className="flex-1 overflow-y-auto custom-scrollbar">
                        {loadingContacts ? (
                            ...
                            <div
                                className="flex flex-col items-center justify-center py-8 px-4 text-center text-muted-foreground">
            ...
            {activeContact ? (
                ...
                /* Empty state */
                <div
                    className="flex-1 flex flex-col items-center justify-center text-muted-foreground bg-muted/10 px-6">
                    <div
