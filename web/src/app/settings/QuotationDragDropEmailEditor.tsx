...
function VariablePicker({ onInsert, dropAlign = 'left', placeholders }: {
  onInsert: (v: string) => void; dropAlign?: 'left' | 'right'; placeholders: { label: string; val: string }[];
}) {
  ...
  return (
    <div className="relative" ref={ref}>
      <button
        ...
        className={`flex items-center gap-1.5 px-2.5 py-1 text-xs font-medium rounded-lg border transition-all ${open
...
function RichTextEditor({
  value,
  onChange,
  variables, // 1. Add this prop
}: {
  value: string;
  onChange: (html: string) => void;
  variables: { label: string; val: string }[]; // 2. Add this type
}) {
  ...
  return (
    <div className="border border-gray-200 rounded-lg overflow-hidden focus-within:ring-2 focus-within:ring-blue-500 focus-within:border-blue-400 transition-shadow">
      ...
      <div className="flex flex-wrap items-center gap-0.5 px-2 py-1.5 bg-gray-50 border-b border-gray-200">
        ...
        <VariablePicker onInsert={insertVariable} dropAlign="right" placeholders={variables} />
      </div>
      ...
      {['h1', 'h2'].includes(block.type) && (
        <div>
          <div className="flex items-center justify-between mb-1">
            ...
            <VariablePicker onInsert={insertVariable} placeholders={variables} />
          </div>
          <input
            ref={inputRef}
            type="text"
...
interface QuotationDragDropEmailEditorProps {
  htmlContent: string;
  subject?: string;
...
export default function QuotationDragDropEmailEditor({ htmlContent, subject, onContentChange, placeholders }: QuotationDragDropEmailEditorProps) {
  // Define VARIABLES here inside the component
  const VARIABLES = placeholders.map(p => ({
    label: p.display_name,
    val: ` [${p.placeholder_key}]`
  }));
