...
import QuotationHtmlEmailEditor from './QuotationHtmlEmailEditor';
...
interface QuotationEmailTemplateEditorProps {
  mode: 'create' | 'edit';
...
export default function QuotationEmailTemplateEditor({ mode, initialTemplate, tenantId, placeholders,
  onCancel, // <--- Add this here
  onSuccess // Destructure here
}: QuotationEmailTemplateEditorProps) {
  ...
  // so internal state (e.g. QuotationDragDropEmailEditor blocks) is preserved across switches.
  ...
  const handleMediaUpload = async (e: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      set('media_url', url);
      // Persist to localStorage so QuotationHtmlEmailEditor can list it
      if (url) {
        try {
  ...
  const handleSave = async (andRedirect = true) => {
    const hasContent = editorMode === 'html' || editorMode === 'dragdrop'
      ...
      : template.body_text.trim().length > 0;
  ...
  const previewHtml = editorMode === 'html' || editorMode === 'dragdrop'
    ? (template.body_html || '')
    : `<p style="font-family:Arial,sans-serif;font-size:15px;line-height:1.7;color:#374151;">${(template.body_text || '').replace(/\n/g, '<br/>')}</p>`;
  ...
  return (
    <div className="h-screen flex flex-col bg-gray-50 overflow-hidden">
      ...
      <header className="flex-shrink-0 bg-white border-b border-gray-200 px-5 py-3 flex items-center gap-3">
        ...
        <div className="flex flex-col min-w-0">
          ...
          <div className="flex items-center gap-1.5 mt-0.5">
            <span className={`w-2 h-2 rounded-full flex-shrink-0 ${template.is_default ? 'bg-green-400' : 'bg-gray-300'}`} />
            <span className="text-xs text-gray-400">{template.is_default ? 'Active' : 'Inactive'}</span>
          </div>
        </div>
          ...
          <div className="relative" onMouseDown={(e) => e.stopPropagation()}>
            ...
            {showSaveMenu && (
              <div className="absolute right-0 top-full mt-1 w-48 bg-white rounded-xl shadow-lg border border-gray-100 py-1 z-50">
                ...
                <button
                  onClick={() => { set('is_default', !template.is_default); setShowSaveMenu(false); }}
                  className="w-full text-left px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50"
                >
                  {template.is_default ? 'Deactivate template' : 'Activate template'}
                </button>
              </div>
            )}
          </div>
      ...
      <div className="flex-shrink-0 bg-white border-b border-gray-200 px-6">
        <div className="flex">
          {(['editor'] as const).map((tab) => (
            <button
              key={tab}
              onClick={() => setActiveTab(tab)}
              className={`py-3.5 px-4 text-sm font-medium border-b-2 transition-colors capitalize ${activeTab === tab
          ...
          <>
                  ...
                  <div className="flex gap-1" onMouseDown={(e) => e.stopPropagation()}>
                    {EDITOR_OPTIONS.map(({ mode: m, label, icon }) => (
                      <button
                        ...
                        className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${editorMode === m
                      ...
                      <div className={editorMode === 'simple' ? 'space-y-4' : 'hidden'}>
                        ...
                        {/* <div className="flex flex-wrap items-center gap-2 px-4 py-3 bg-white rounded-xl border border-gray-200">
                          ...
                          {placeholders.map((ph) => (
                            <button
                              key={ph.id}
                              onClick={() => set('body_text', (template.body_text || '') + ' [' + ph.placeholder_key + ']')}
            ...
            <aside className="w-1/2 bg-white overflow-y-auto flex flex-col">
              <div className="p-6 space-y-5 flex-1 max-w-2xl mx-auto w-full">
                ...
                <div className="flex items-center justify-between py-2">
                  <div>
                    ...
                    <p className="text-xs text-gray-400">{template.is_default ? 'DEFAULT' : ''}</p>
                  </div>
                  <button
                    onClick={() => set('is_default', !template.is_default)}
                    className={`relative w-11 h-6 rounded-full transition-colors ${template.is_default ? 'bg-green-500' : 'bg-gray-300'}`}
                  >
                    <span className={`absolute top-1 w-4 h-4 bg-white rounded-full shadow transition-transform ${template.is_default ? 'translate-x-6' : 'translate-x-1'}`} />
                  </button>
                </div>
              ...
              <div className="border-t border-gray-100 p-5 space-y-2 max-w-2xl mx-auto w-full">
                ...
                <button
                  type="button"
                  onClick={onCancel} // <--- Call the prop here
                  className="w-full text-center py-2.5 border border-gray-200 text-sm text-gray-600 rounded-xl hover:bg-gray-50 transition-all"
                >
                  Cancel
                </button>
              </div>
          ...
          <div className="bg-white rounded-2xl shadow-2xl flex flex-col w-full max-w-3xl max-h-[90vh]">
            ...
            <div className="flex-1 overflow-auto p-6 bg-gray-50">
              <div className={`mx-auto bg-white rounded-xl shadow-sm transition-all duration-300 ${device === 'mobile' ? 'max-w-[375px]' : device === 'tablet' ? 'max-w-[768px]' : 'max-w-full'}`}>
                <QuotationEmailPreview htmlContent={previewHtml} subject={template.subject} showDeviceSelector={false}
                  placeholders={placeholders} />
              </div>
            </div>
          </div>
