...
export const updateStage = async (stageKey: string, updates: Partial<Stage> | string): Promise<Stage> => {
  ...
  const response = await api.put(`/api/leads/stages/${stageKey}`, updateData);
  // Invalidate cache
  stagesCache = null;
  return response.data as Stage;
};
...
export const deleteStage = async (stageKey: string): Promise<void> => {
  await api.delete(`/api/leads/stages/${stageKey}`);
  // Invalidate cache
  stagesCache = null;
};
...
export const fetchAttachments = async (leadId: string | number): Promise<unknown[]> => {
  try {
    const { data } = await api.get(`/api/leads/${leadId}/attachments`);
    return data as unknown[];
  } catch (error) {
    const err = error as Error;
...
export const uploadAttachment = async (leadId: string | number, file: File): Promise<unknown> => {
  try {
    ...
    const { data } = await api.post(`/api/leads/${leadId}/attachments`, formData, {
      headers: {
...
// Default export - Pipeline Service SDK
const pipelineService = {
  // Pipeline Board
  fetchPipelineData,
  fetchPipelineOverview,
  fetchPipelineBoard,
  fetchDealsPipelineBoard,
